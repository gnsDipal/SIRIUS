...
import styles from './AddEditPersonalGoal.module.scss';
import { IAddEditPersonalGoalProps, IAddEditPersonalGoalState } from './IAddEditPersonalGoal.Props';
...
const AssignToOptions : IDropdownOption[] = [
  // { key: 'AssignToMembers', text: 'AssignToMembers', itemType: DropdownMenuItemType.Header },
  // { key: 'AssignToMembers1', text: 'Mangesh Bhunde'},
  // { key: 'AssignToMembers2', text: 'Dipal Bhavsar' },
  // { key: 'AssignToMembers3', text: 'Vrushali Agarwal' }
];
...
export default class AddEditPersonalGoal extends React.Component<IAddEditPersonalGoalProps, IAddEditPersonalGoalState, {}> {
  ...
  constructor(props: IAddEditPersonalGoalProps, state:IAddEditPersonalGoalState) {
    ...
    //this.onChangeAssignToHandle = this.onChangeAssignToHandle.bind(this);
  }

  componentDidMount()
  {
    //this._getAssignToOptionsData();
  ...
  _SaveButtonClicked = () =>{
    ...
    // onChangeAssignToHandle = async (selectedAssignTo:any)=> {
    //   await this.setState({
    //     selectedAssignTo :  selectedAssignTo.text,
    //   });
    //   //console.log("selectedAssignTo ==> ", this.state.selectedAssignTo);
    // }
  ...
  displayEditStatusIdData =  async () =>
      {
         ...
         headers.append("accept", "application/json;odata.metadata=none");
           ...
           .get(`${this.props.siteurl}/_api/web/lists/getbytitle('GoalPersonal')/items?$select=ID,Title,Goal,Interval,StatusPercentage&$filter=Id eq '${this.props.RequireGoalStatusId}'`,
...
_SaveAddPersonalGoal(){
  alert( "Personal Goal Data Saved");
  this.setState({
    AddEditGoalsFormDisplay:false,
    IntervalDataDisplay:true,
  });
    ...
    const spOpts: string = JSON.stringify({
      ...
      //'AssignTo':this.state.selectedAssignTo,
      'AssignToId':this.props.currentUserId,
      'StatusPercentage':this.state.selectedStatusPercentage,
    });

    this.props.spHttpClient.post(`${this.props.webUrl}/_api/web/lists/getbytitle('GoalPersonal')/items`,
    SPHttpClient.configurations.v1,
    {
        headers: {
...
_SaveEditPersonalGoal(){
  alert( "Personal Goal Data Updated");
  this.setState({
    AddEditGoalsFormDisplay:false,
    IntervalDataDisplay:true,
  });
  ...
  headers.append("accept", "application/json;odata.metadata=none");
   ...
   this.props.spHttpClient
    .get(`${this.props.siteurl}/_api/web/lists/getbytitle('GoalPersonal')/items('${this.props.RequireGoalStatusId}')?$select=ID,Title,Goal,Interval,StatusPercentage&$filter=Id eq '${this.props.RequireGoalStatusId}'`,
      SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then((result: SPHttpClientResponse) => {
  ...
  this.props.spHttpClient.post(`${this.props.webUrl}/_api/web/lists/getbytitle('GoalPersonal')/items(${this.props.RequireGoalStatusId})`,
  SPHttpClient.configurations.v1,
  {
      headers: {
          'Accept': 'application/json;odata=nometadata',
...
public render(): React.ReactElement<IAddEditPersonalGoalProps> {
 return (
  <div className={ styles.addEditPersonalGoal }>
    {/* <div className={ styles.container }> */}
      { (this.state.AddEditGoalsFormDisplay === true) &&
        <div className={ styles.FormDisplay }>
          <div className="ms-Grid" dir="ltr">
               {/* Form Header information */}
               {((this.props.openAddEditForm === 1) ? <h3>Add Personal Goal Form</h3>
                : (this.props.openAddEditForm === 2) ? <h3>Edit Personal Goal Form</h3>
                : "")}
               ...
               <div className="ms-Grid-row" style={{marginBottom:'20px'}}>
                 <div className="ms-Grid-col ms-lg8 ms-sm8">
                   {/* Save Button information (Save New Data and Save Edit Data)*/}
                      {((this.props.openAddEditForm === 1) ?
                          <PrimaryButton onClick={() => this._SaveAddPersonalGoal()} ><h3>Save</h3></PrimaryButton>
                      :(this.props.openAddEditForm === 2) ?
                          <PrimaryButton onClick={() => this._SaveEditPersonalGoal()} ><h3>Save</h3></PrimaryButton>
                      :"")}
