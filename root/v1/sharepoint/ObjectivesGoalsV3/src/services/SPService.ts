...
export default class SPService{
    ...
    public async getIntervalsData():Promise<any> {
        let result = await this.web.lists.getByTitle('Intervals').items.select("Title").filter("IsActive eq '1'").orderBy("ID",false).get();
        return await Promise.resolve(result);
    }
}