...
/* Load count of my requested issues */
public async loadMyRequestsViewCount():Promise<MyRequestedEachPlateData[]>{
    ...
    this.uniqueDeptList = [];
      let result = await this.web.lists.getByTitle(this.mainProp.employeeRequestListName).items.select('Department','AttachmentFiles').expand('AttachmentFiles').filter(`Author eq ${this.loggedInUserId}`).orderBy("ID",false).get();
      ...
      if(result.length>0){
       ...
       /* Waiting for all counts complete their promise through for loop */
