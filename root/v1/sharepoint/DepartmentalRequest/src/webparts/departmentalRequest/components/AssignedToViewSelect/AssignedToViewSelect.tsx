...
export default class AssignedToViewSelect extends React.Component<IAssignedToViewSelectProps, IAssignedToViewSelectState> {
 ...
 private initialDeptGetMethod() {
  ...
  this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmpReq')/items?$select=*,AttachmentFiles&$expand=AttachmentFiles&$filter=ReAssignToId eq ${this.props.currentUserId}&$orderby=ID desc`,
...
private async getInProcessRequestsCount(deptName){
  ...
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmpReq')/items?$select=*&$filter=((ReAssignToId eq ${this.props.currentUserId}) and (Status eq 'In Process') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
...
private async getclosedRequestsCount(deptName){
  ...
  headers.append("accept", "application/json;odata.metadata=none");
  return this.props.spHttpClient
    .get(`${this.props.webUrl}/_api/web/lists/GetByTitle('EmpReq')/items?$select=*&$filter=((ReAssignToId eq ${this.props.currentUserId}) and (Status eq 'Completed') and (Department eq '${deptName}'))&$orderby=ID desc`,
    SPHttpClient.configurations.v1, {
      headers: headers
    })
    .then(result=>{
